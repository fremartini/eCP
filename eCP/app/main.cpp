...
int main(int argc, char* argv[])
{
  ...
  int b = 2;           // number clusters to search6
  ...
  if (argc > 1 && argc % 2 != 0) {
    for (int i = 1, j = 2; j != argc + 1; i += 2, j += 2) {
      ...
      if (flag == "-f") {
        ...
        S = utility::load_hdf5_file(file, dataset);
        dataset = "test";
        queries = utility::load_hdf5_file(file, dataset);
        p = S.size();
        hdf5 = true;
      }
      else if (flag == "-k") {
  ...
  if (!hdf5) {
    std::cout << "Generating " << p << " descriptors with " << d << " dimensions and values up to " << r
              << std::endl;
    S = utility::generate_descriptors(p, d, r);
    queries = utility::generate_descriptors(qs, d, r);
  }
