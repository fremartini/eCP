...
namespace distance
{
    ...
    inline float euclidean_distance(const float* a, const float* b)
    {
        ...
        {
			sums[0] += ((a[i] - b[i]) * (a[i] - b[i])) + ((a[i+1] - b[i+1]) * (a[i+1] - b[i+1]));
			sums[1] += ((a[i+2] - b[i+2]) * (a[i+2] - b[i+2])) + ((a[i+3] - b[i+3]) * (a[i+3] - b[i+3]));
			sums[2] += ((a[i+4] - b[i+4]) * (a[i+4] - b[i+4])) + ((a[i+5] - b[i+5]) * (a[i+5] - b[i+5]));
			sums[3] += ((a[i+6] - b[i+6]) * (a[i+6] - b[i+6])) + ((a[i+7] - b[i+7]) * (a[i+7] - b[i+7]));
        }
