...
Index* eCP_Index(const std::vector<std::vector<float>>& descriptors, unsigned int L, unsigned int metric) {
  ...
  const auto sample_size =
      std::ceil(std::pow(descriptors.size(), ((L / (L + 1.00)))));  // The first 'sample_size' elements are used as leaders for the bottom level
...
std::pair<std::vector<unsigned int>, std::vector<float>> query(Index* index, std::vector<float> query, unsigned int k, unsigned int b) {
  ...
  for (auto it = std::make_move_iterator(nearest_points.begin()), end = std::make_move_iterator(nearest_points.end()); it != end; ++it) {
