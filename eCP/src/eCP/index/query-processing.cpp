...
std::vector<std::pair<unsigned int, float>> k_nearest_neighbors(std::vector<Node>& root, float*& query, const unsigned int k, const unsigned int b = 1,
                                                                unsigned int L = 1) {
...
unsigned index_to_max_element(std::vector<std::pair<unsigned int, float>>& point_pairs) {
  assert(!point_pairs.empty() &&
         "point_pairs must contain at least a single element.");  // FIXME: Make sure that asserts are not part of release builds somehow
 ...
 */
bool smallest_distance(std::pair<unsigned int, float>& a, std::pair<unsigned int, float>& b) { return a.second < b.second; }
